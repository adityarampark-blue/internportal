...
// Helper function to get next intern ID
async function getNextInternId() {
  ...
  return `IN${String(counter.seq).padStart(3, '0')}`;
}
...
// POST - Create new intern with auto-generated ID
router.post('/', async (req, res) => {
  try {
    const data = req.body;
    console.log('Creating intern with data:', data);
    ...
    // Check if ID already exists (should not happen with counter)
    const existing = await Intern.findOne({ id: nextId });
    if (existing) {
      // If somehow exists, try to get next available
      const allInterns = await Intern.find().sort({ createdAt: -1 });
      let maxNum = 0;
      allInterns.forEach(intern => {
        const match = intern.id.match(/^IN(\d+)$/);
        if (match) {
          maxNum = Math.max(maxNum, parseInt(match[1]));
        }
      });
      const newId = `IN${String(maxNum + 1).padStart(3, '0')}`;
      const internData = { ...data, id: newId };
      const intern = new Intern(internData);
      await intern.save();
      console.log('Created intern with fallback ID:', intern);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Delete the user account for this intern too
    let userDeleted = null;
    const userById = await User.findOneAndDelete({ id });
    if (userById) {
      userDeleted = userById;
    ...
    // Note: Counter is NOT decremented to maintain unique IDs
    // This ensures future interns get sequential IDs without reusing deleted ones
...
// GET next available intern ID (for preview)
